...
var ft string = `{{ extends "block_base" }}
...
const instructionString = {{.BackQuote}}%s [OPTIONS...] {COMMAND} ...
...
func NewCommand(tag, use string, priority int, efn ExecutionFunc, fs *FlagSet) Command {
	return &command{"", tag, use, priority, efn, fs}
}
...
func UnquoteUsage(flag *Flag) (name string, usage string) {
	...
	for i := 0; i < len(usage); i++ {
		if usage[i] == '{{.BackQuote}}' {
			for j := i + 1; j < len(usage); j++ {
				if usage[j] == '{{.BackQuote}}' {
					name = usage[i+1 : j]
